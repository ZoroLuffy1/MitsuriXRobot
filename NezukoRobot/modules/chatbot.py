...
from NezukoRobot import BOT_USERNAME as bu
from NezukoRobot import BOT_ID, pbot, arq
from NezukoRobot.ex_plugins.chatbot import add_chat, get_session, remove_chat
from NezukoRobot.utils.pluginhelper import admins_only, edit_or_reply
...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            ...
            return
        await lel.edit(f"Nezuko AI Actived by {message.from_user.mention()} for users in {message.chat.title}")
        ...
        if not Escobar:
            ...
            return
        await lel.edit(f"Nezuko AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}")
...
@pbot.on_message(filters.regex("Nezuko|Nezuko|robot|Nezuko|vain") & ~filters.bot & ~filters.via_bot  & ~filters.forwarded & ~filters.reply & ~filters.channel & ~filters.edited)
async def inuka(client, message):
    msg = message.text
    if msg.startswith("/") or msg.startswith("@"):
        message.continue_propagation()
